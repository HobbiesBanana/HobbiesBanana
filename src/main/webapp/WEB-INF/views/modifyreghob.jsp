...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="resources/css/meetinginsertform.css">
  ...
  <nav class="navbar navbar-expand-lg navbar-dark ftco_navbar bg-dark ftco-navbar-light" id="ftco-navbar">
    ...
    <section id="main-container">
      <div class="container">
        <form method="post" action="modifyhob.do" enctype="multipart/form-data">
          <input type="hidden" name="meetingId" value="${ viewmodreghob.MEETING_ID }"/>
          <div class="category">
            ...
            <div class="category-btn">
              ...
              <div>
                <select id="category-select" name="code" required>
                  ...
                  <option value="A001" <c:if test="${ viewmodreghob.CODE == 'A001' }">selected</c:if>>문화, 예술</option>
                  <option value="A002" <c:if test="${ viewmodreghob.CODE == 'A002' }">selected</c:if>>레저, 엑티비티</option>
                  <option value="A003" <c:if test="${ viewmodreghob.CODE == 'A003' }">selected</c:if>>푸드, 드링크</option>
                  <option value="A004" <c:if test="${ viewmodreghob.CODE == 'A004' }">selected</c:if>>독서, 음악</option>
                  <option value="A005" <c:if test="${ viewmodreghob.CODE == 'A005' }">selected</c:if>>성장, 자기개발</option>
                  <option value="A006" <c:if test="${ viewmodreghob.CODE == 'A006' }">selected</c:if>>여행, 나들이</option>
                  <option value="A007" <c:if test="${ viewmodreghob.CODE == 'A007' }">selected</c:if>>그 외의 취미</option>
                </select>
              </div>
            </div>
          </div> <!-- category end -->
            <div class="main-content">
              ...
              <div class="title-contain">
                <input type="text" name="title" value="${viewmodreghob.TITLE}" required/>
              </div>
              <div class="introduce-cotain">
                ...
                <input type="file" name="firstFile" value="${ viewmodreghob.FIRST_IMG }"/>
                <input type="file" name="secondFile" value="${ viewmodreghob.SECOND_IMG }"/>
                <input type="file" name="thirdFile" value="${ viewmodreghob.THIRD_IMG }"/><br/>
                <span class="middle-text">취미방을 소개하는 소개글을 작성해주세요!</span><br/>
                <textarea name="content" required>${viewmodreghob.CONTENT} </textarea><br/>
                ...
                <div class="date-contect">
                  ...
                  <div class="maptext-contain">
                    ...
                    <input type="text" id="address_kakao" name="location" value="${viewmodreghob.LOCATION}" readonly required/><br/>
                    <span class="middle-text">상세주소</span><br/>
                    <input type="text" name="detailLocation" value="${viewmodreghob.DETAIL_LOCATION}" required/><br/>
                    <span class="middle-text">날짜와 시간을 설정 해주세요!</span><br/>
                    <input type="date" name="meetingDate" value="${viewmodreghob.DAT}" required/><br/>
                    <input type="time" name="meetingTime" value="${viewmodreghob.TIME}" required/>
                  </div>
                </div>
                <div class="membercount-contain">
                  ...
                  <div class="btn-count">
                    ...
                    <input type="number" class="count" name="memberCount" value="${viewmodreghob.MEMBER_COUNT}" required/>
