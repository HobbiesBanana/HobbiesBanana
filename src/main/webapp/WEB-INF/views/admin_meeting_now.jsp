  ...
  <body>
    ...
    <section class="hero-wrap hero-wrap-2 ftco-degree-bg js-fullheight" data-stellar-background-ratio="0.5">
        <div  style="background-image:url('resources/images/관리자배경3.jpg'); background-size: cover;" class="row no-gutters slider-text js-fullheight align-items-end justify-content-center">
          ...
          <h1>관리자 페이지</h1><br/>
           <h5>게시물 관리 > 현재 진행중인 게시물 조회</h5>
           <br/><br/><br/>
          </div>
        </div>
    </section>
 
	<form action="meetingNow.do" method="post">
		...
		<div class="adminmain">
			<div>
        		...
        		<tr>
        			...
        			<tbody>
        		    <c:forEach items="${meetingNow}" var="meetingNow">
        			<tr>
        				<td>${meetingNow.MEETING_ID}</td>
        				<td>${meetingNow.TITLE}</td>
        				<td>${meetingNow.CONTENT}</td>
        				<td>${meetingNow.NAME}</td>
        				<td><a href="meetinglist2.do?meetingId=${meetingNow.MEETING_ID}"><input type="button" value="상세보기" style="border:none;"/></a></td>
        			</tr>
        		</c:forEach>
        		</tbody>
        	</table> 
