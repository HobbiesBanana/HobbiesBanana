       ...
       <ul class="list_contain">
        <br/>
       		<table id="reglist" border="1">
             	    <tr class="tablebar" align = "center" bgcolor ="#d4ca68">
					<th>No</th>
					<th>취미명</th>
					<th>승인여부</th>
					<th>삭제하기</th>
					...
					<c:forEach items="${selectreqhob}" var="reqhob">
					  <tr align ="center"><!-- 첫번째 줄 시작 -->
					   <td>${reqhob.ROWNUM}</td>
					   <td>${reqhob.TITLE}</td>
					   <td>${reqhob.APPROVAL}</td>
					   <td><a href="deletereqhob.do?applyId=${reqhob.APPLY_ID }"><input id="deleteBtn" type="button" class="button2" value="삭제"></a></td>
					</tr><!-- 첫번째 줄 끝 -->  
					</c:forEach> 
			</table>
		     <br/>
