       ...
       <ul class="list_contain">
        <br/>
       		<table id="reglist" border="1">
					...
					<c:forEach items="${guest}" var="guestlist">
					  <tr align ="center"><!-- 첫번째 줄 시작 -->
					   <td>${guestlist.ROWNUM}</td>
					   <td>${guestlist.TITLE}</td>
					   <td>${guestlist.NAME}</td>
					   <td>${guestlist.GENDER}</td>
					   <td>${guestlist.AGE}</td>
					   <td>${guestlist.EMAIL}</td>
					   <td><a href="guestagree.do?memberId=${ sessionScope.memberId }"><input id="modifyBtn" type="button" class="button2" value="수락"></a></td>
					</tr><!-- 첫번째 줄 끝 -->  
					</c:forEach>
			</table>
		    <br/>
